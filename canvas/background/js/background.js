    ...
    function Particle()
    {
        ...
        this.move = function()
        {
            ...
            if(this.type === 0) {
                ...
                context.moveTo(this.x - 10 * this.size, this.y - 10 * this.size);
                context.lineTo(this.x + 10 * this.size, this.y + 10 * this.size);
                context.moveTo(this.x + 10 * this.size, this.y - 10 * this.size);
                context.lineTo(this.x - 10 * this.size, this.y + 10 * this.size);
...
function rotateCoord(x, y, grad) {
  const angle = grad * Math.PI / 180,
	r = Math.sqrt( x**2 + y **2 ),
  // высчитываем "начальный" поворот
  a = Math.atan( y / x ),
